...
public class FetchOnlineUserTask extends BaseTask<Myself, Void, List<Myself>> {
	...
	public List<Myself> connectServer(Myself info) {
		try {
			channel = bootStrap.connect(Const.NEETY_IP, Const.NETTY_PORT)
					.sync().channel();
		...
		if (channel != null && channel.isRegistered()) {
			...
			final String url = Const.BASE_URL + "friend?n="
					+ info.getChannelId();
			...
			ResponseEntity<Myself[]> resp = rest.exchange(url, HttpMethod.GET,
					requestEntity, Myself[].class);
